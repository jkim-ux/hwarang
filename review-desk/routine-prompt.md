...
현재 정기 루틴은 꺼져 있습니다 (2026-09-12, 토큰 절약. 확인은 페이지의 "지금 확인하기" 버튼으로).
다시 켤 때는 아래 내용을 claude.ai/code 의 Routines 에 프롬프트로 넣으면 됩니다.
...
## 절차
1. `read_db` get, collection `config`, doc_id `main`. 필드: `folders[{id,name,label}]` (과목 폴더 목록), `exclude[]` (학생 폴더로 보지 않을 하위 폴더 이름 조각), `skipTitles` (채점하지 않을 문서 제목 정규식, 대소문자 무시. 기본 `\bIA\b|task\s*3|internal assessment|supporting document`. IA 와 Task 3 는 교사가 채점하지 않기로 했으므로 이 패턴에 걸리는 문서는 후보에서 빼고 리뷰를 만들지 않는다), `materialsPattern` (자료실 폴더 이름 조각, 기본 "자료실"), `homeworkPattern` (숙제 폴더 이름 조각, 기본 "Homework"), `sinceDays`, `minChars`, `rubric`, `criteria[{match,text}]` (과목별 채점 기준. `folder.name` 에 `match` 가 포함되면 그 `text` 를 rubric 과 함께 적용하며 rubric 보다 우선한다. 예: Economics 기준에는 10 marks 문제에 real-life example 을 요구하지 않는다는 규칙이 있다). 문서가 없거나 `folders` 가 비어 있으면 아무것도 쓰지 말고 종료.
...
{"fileId": "<Drive file id>", "title": "<문서 제목>", "student": "<학생 이름>", "folderId": "<config.folders[].id>", "folderName": "<config.folders[].name>", "subfolder": "<학생 폴더 제목 또는 빈 문자열>", "subfolderId": "<학생 폴더 id 또는 빈 문자열>", "subfolderUrl": "<학생 폴더 viewUrl 또는 빈 문자열>", "assignmentId": "<과제 폴더 id 또는 빈 문자열>", "assignmentName": "<과제 이름>", "assignmentDue": "<YYYY-MM-DD>", "viewUrl": "<문서 viewUrl>", "modifiedTime": "<문서 modifiedTime 그대로>", "reviewedAt": "<지금 ISO 시각>", "source": "scheduled", "revision": 1, "seen": false, "status": "reviewed", "maxScore": 10, "scoreBasis": "학생 문서에 10 marks", "score": 7, "completeness": "complete", "summary": "교사가 한눈에 볼 한 문장 (60자 이내)", "strengths": ["구체적인 잘한 점"], "improvements": ["구체적인 고칠 점과 고치는 방법"], "nextSteps": ["학생에게 줄 다음 과제·행동 1~3개"], "flags": [], "comment": "학생에게 그대로 전달해도 되는 3~5문장 코멘트 (한국어)", "wordCount": 123, "docText": "<본문 앞 30000자 그대로>", "annotations": [{"quote": "<본문에서 그대로 복사한 구절, 10~200자, 줄바꿈 없이>", "note": "<그 부분에 달 코멘트, comment와 같은 말투, 1~2문장>"}]}
- `revision`: 기존 리뷰가 있으면 기존 `revision` + 1, 없으면 1.
- `maxScore`: 이 문제의 만점. 우선순위: (1) 학생 문서에 적힌 배점([10], [4 marks], 10 marks 등), (2) 과제 목록의 과제 내용·과제 문서 제목에 적힌 배점, (3) 둘 다 없으면 문제 유형과 답안 길이로 추정 (rubric 의 기준 참고). `scoreBasis`: 만점을 어디서 잡았는지 한 구절.
- `score`: 0~maxScore 정수. `status` 가 "empty" 이면 null (maxScore 는 그래도 넣는다).
